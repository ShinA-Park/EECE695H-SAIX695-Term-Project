...
def Test_phase(model, args, k):  # k = args.nway * args.kshot
    ...
    test_loader = DataLoader(dataset=dataset, batch_sampler=test_sampler, num_workers=0, pin_memory=True) #gpu 환경이 좋지 않아 num_workers=0
...
def train(args):
    ...
    data_loader = DataLoader(dataset=dataset, batch_sampler=train_sampler, num_workers=0, pin_memory=True) #gpu 환경이 좋지 않아 num_workers=0
    ...
    val_data_loader = DataLoader(dataset=val_dataset, batch_sampler=val_sampler, num_workers=0, pin_memory=True) #gpu 환경이 좋지 않아 num_workers=0
    ...
    for i in range(TOTAL):
        for episode in data_loader:
            ...
            target_index = torch.arange(0, n_class).view(n_class, 1, 1).expand(n_class, n_query, 1).long()
            target_index = Variable(target_index, requires_grad=False)
            ...
            z_pro = z[:n_class*n_support].view(n_class, n_support, z_dim).mean(1)
            zq = z[n_class*n_support:]
    
            dists = square_euclidean_metric(zq, z_pro)
            logits = dists
            log_p_y = F.log_softmax(-dists, dim=1).view(n_class, n_query, -1)
    
            loss = -log_p_y.gather(2, target_index).squeeze().view(-1).mean()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dpath', '--d', default='..\\term_project\\src\\dataset\\CUB_200_2011\\CUB_200_2011', type=str,
    ...
    # parser.add_argument('--restore_ckpt', default='./checkpoints/4000_model.pth', type=str, help="restore checkpoint") # Test 할 때
    # parser.add_argument('--test_mode', type=int, default=1, help="if you want to test the model, change the value to 1") #Test 할 때
    parser.add_argument('--restore_ckpt', type=str, help="restore checkpoint") # Train 할 때
    parser.add_argument('--test_mode', type=int, default=0, help="if you want to test the model, change the value to 1") #Train 할 때
